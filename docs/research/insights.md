...
- **Lição:** Para distribuição futura (Docker ou exe), o caminho do Tesseract não pode ser _hardcoded_. Precisamos verificar variáveis de ambiente primeiro.
...
- **Solução:** Implementamos um _buffer_ em disco (`temp_email/`). O script baixa os bytes, materializa um arquivo temporário, processa e (opcionalmente) deleta. Isso desacopla a lógica de rede da lógica de processamento.
...
- Quando um campo "sumiu", use `scripts/inspect_pdf.py --raw` para confirmar como o PDF está quebrando palavras e se o extrator correto está sendo selecionado.
